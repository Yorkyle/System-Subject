...
export function render(state, els, dispatch){
  ...
  // Tabs appear only once the case is underway (keeps immersion during boot)
  ...
  state.feed.slice(-250).forEach((m)=>{
    ...
    const textLine = `<div class="text">${esc(m.text).replace(/\n/g, "<br>")}</div>`;
  ...
  if(state.screen === "home"){
    ...
    else if(state.rightTab === "manual") renderManual(state, els.rightPanel);
...
function renderActions(state, mount, dispatch){
  ...
  // If an intervention is being confirmed, show the confirmation prompt first
  if(state.pendingIntervention){
    const c = state.pendingIntervention;
    const confirm = document.createElement("div");
    confirm.className = "card";
    confirm.innerHTML = `<div class="cardTitle">CONFIRMATION</div>
      <div class="small mono">SUDO</div>
      <div class="small" style="white-space:pre-wrap; line-height:1.45">
You can proceed if you wish.
But understand: this path is difficult to undo.
      </div>
      <hr class="sep"/>
      <div class="small mono">Selected: ${esc(c.title)}</div>`;
    const row = document.createElement("div");
    row.className = "btnRow";
...
function renderManual(state, mount){
      const box = document.createElement("div");
      box.className = "card";
      box.innerHTML = `<div class="cardTitle">OBSERVER PROTOCOL</div>
        <div class="small" style="white-space:pre-wrap; line-height:1.55">
You will receive fragments.
...
Your role is not to be right.
Your role is to reduce harm with the smallest possible action.
